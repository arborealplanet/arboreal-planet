...
type Post={
  ...
  type:string;section:string;body:string;tags:string[];media_urls:string[];video_url:string|null;
...
export function CommunityFeed({initialQuery="",initialSection=""}:{initialQuery?:string;initialSection?:string}){
  ...
  const visible=useMemo(()=>{
    ...
    return rows.filter(post=>{
      ...
      if(!q)return true;
      return [post.body,post.display_name,post.username,post.section,post.species_name,post.plant_name,...(post.tags??[])].some(value=>String(value??"").toLowerCase().includes(q));
    });
  },[rows,type,section,query]);
  ...
  return <div data-community-feed className="space-y-4 scroll-mt-28">
    <div className="panel rounded-[26px] p-3 sm:p-4">
      <div className="flex flex-col gap-3">
        ...
        <div className="grid gap-2 sm:grid-cols-[minmax(0,.75fr)_minmax(0,1.25fr)]">
          <select value={section} onChange={event=>setSection(event.target.value)} className="w-full rounded-xl border border-white/[.07] bg-[#08130e] px-3 py-3 text-sm text-white/65 outline-none focus:border-emerald-300/20">
            <option value="">All sections</option>
            {COMMUNITY_SECTION_GROUPS.map(group=><optgroup key={group.key} label={group.heading}>{group.sections.map(item=><option key={item} value={item}>{item}</option>)}</optgroup>)}
          </select>
          <div className="relative"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-sm text-white/25">⌕</span><input value={query} onChange={event=>setQuery(event.target.value.slice(0,120))} placeholder="Search keepers, sections, references, tags or posts" className="w-full rounded-xl border border-white/[.07] bg-black/15 py-3 pl-10 pr-4 text-sm text-white/70 outline-none placeholder:text-white/25 focus:border-emerald-300/20"/></div>
        </div>
        {query||section?<div className="flex items-center justify-between gap-3 text-[10px] text-white/30"><span>{section?`Section: ${section}`:"All sections"}{query?` · Search: “${query}”`:""}</span><button type="button" onClick={()=>{setQuery("");setSection("")}} className="font-bold text-emerald-200/55 hover:text-emerald-200/80">Clear filters</button></div>:null}
      </div>
    </div>

    {authRequired?<div className="panel rounded-[26px] p-10 text-center"><div className="font-semibold text-white/72">Sign in to use your Following feed.</div><p className="mt-2 text-xs text-white/45">Explore stays public. Following can include keepers, animals, plants, topics and sections you choose.</p><Link href="/login?next=/community" className="btn-primary mt-4 inline-block">Sign in</Link></div>
    :loading?<div className="panel rounded-[26px] p-10 text-center text-sm text-white/42">Loading community…</div>
    :!visible.length?<div className="panel overflow-hidden rounded-[26px] text-center"><img src="/community/welcome.jpg" alt="" className="h-44 w-full border-b border-white/[.055] object-cover"/><div className="p-12"><div className="font-semibold text-white/65">{mode==="following"?"Your Following feed is quiet.":"No posts match this view."}</div><p className="mt-2 text-xs text-white/42">{mode==="following"?"Follow keepers, animals, plants, topics or sections to personalize this feed.":"Try another section, filter or search."}</p>{mode==="following"?<Link href="/interests" className="primary-action mt-4 inline-block">Choose interests</Link>:null}</div></div>
    :visible.map(post=>{
      ...
      return <article id={post.id} key={post.id} className="panel overflow-hidden rounded-[28px] transition hover:border-white/[.11]">
        <div className="flex items-start gap-3 p-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <div className="min-w-0 flex-1">{author?<Link href={author} className="text-sm font-semibold text-white/82 hover:text-emerald-200">{post.display_name||post.username||"Keeper"}</Link>:<div className="text-sm font-semibold text-white/82">{post.display_name||post.username||"Keeper"}</div>}<div className="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-white/36"><span className="font-bold uppercase tracking-[.11em] text-emerald-300/55">{post.type.replaceAll("_"," ")}</span><span>·</span><button type="button" onClick={()=>setSection(post.section)} className="font-semibold text-emerald-100/55 hover:text-emerald-100/80">{post.section}</button><span>·</span><span>{new Date(post.created_at).toLocaleString()}</span></div></div>
          {viewerId===post.author_id&&<CommunityPostOwnerMenu postId={post.id} body={post.body} section={post.section} tags={post.tags??[]} videoUrl={post.video_url}/>}
        </div>
