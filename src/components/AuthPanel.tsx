...
export function AuthPanel() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === "signup" && data.needsConfirmation) { setMessage("Account created. Check your email to confirm it, then come back and sign in."); setMode("login"); return; }
  ...
  return <div className="panel mx-auto max-w-xl overflow-hidden rounded-[30px]">
    <div className="grid grid-cols-2 border-b border-white/[.06] p-2">
      <button onClick={() => setMode("login")} className={`rounded-xl px-4 py-3 text-xs font-black uppercase tracking-[.12em] ${mode === "login" ? "bg-emerald-300 text-[#06100c]" : "text-white/35"}`}>Log in</button>
      <button onClick={() => setMode("signup")} className={`rounded-xl px-4 py-3 text-xs font-black uppercase tracking-[.12em] ${mode === "signup" ? "bg-emerald-300 text-[#06100c]" : "text-white/35"}`}>Create account</button>
    </div>
