    ...
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-6 sm:py-12">
      ...
      {visibleStories.length ? <>
        ...
        <div ref={track} role="region" aria-label="Reptile news stories" tabIndex={0} className="mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3 focus-visible:outline-2 focus-visible:outline-emerald-300">
          {visibleStories.map((story) => <article key={story.url} className="panel-soft flex min-h-[180px] w-[min(78vw,300px)] shrink-0 snap-start flex-col rounded-2xl p-5">
            ...
            <h3 className="mt-4 text-base font-semibold leading-snug text-white">{story.title}</h3>
            <a href={story.url} target="_blank" rel="noopener noreferrer" className="mt-auto pt-4 text-xs font-bold text-emerald-200 hover:text-white focus-visible:outline-2 focus-visible:outline-emerald-300" aria-label={`Read ${story.title} at ${story.publisher} (opens in a new tab)`}>Read at {story.publisher} ↗</a>
          </article>)}
        </div>
