    ...
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-6 sm:py-12">
      ...
      {visibleStories.length ? <>
        ...
        <div ref={track} role="region" aria-label="Reptile news stories" tabIndex={0} className="mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3 focus-visible:outline-2 focus-visible:outline-emerald-300">
          {visibleStories.map((story) => <article key={story.url} className="panel-soft flex min-h-[250px] w-[min(78vw,300px)] shrink-0 snap-start flex-col overflow-hidden rounded-2xl">
            <div className="relative h-24 shrink-0 overflow-hidden bg-[#14251a]">
              <Image src="/abb-site/assets/animals.png" alt="" fill sizes="300px" className="object-cover object-center opacity-80" />
            ...
            <div className="flex flex-1 flex-col p-4">
              ...
              <a href={story.url} target="_blank" rel="noopener noreferrer" className="mt-auto pt-4 text-xs font-bold text-emerald-200 hover:text-white focus-visible:outline-2 focus-visible:outline-emerald-300" aria-label={`Read ${story.title} at ${story.publisher} (opens in a new tab)`}>Read original at {story.publisher} ↗</a>
            </div>
          </article>)}
        </div>
        <p className="mt-2 text-[11px] leading-5 text-white/40">Five recent headlines from the reptile news feeds. Open the news desk for the full list and source details.</p>
