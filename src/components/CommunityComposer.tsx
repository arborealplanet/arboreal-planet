...
import { COMMUNITY_SECTION_GROUPS, LEGACY_COMMUNITY_TOPICS } from "@/lib/community-sections";
...
export function CommunityComposer(){
  ...
  const hasContent=text.trim().length>0||files.length>0||video.trim().length>0;
  const ready=Boolean(section)&&hasContent;
  ...
  async function publish(){
    ...
    try{
      ...
      const response=await fetch("/api/community/posts",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({type,section,body:text,tags:selectedTags,media_urls,video_url:video,species_id:speciesId||null,plant_id:plantId||null}),
      });
      ...
      setText("");setSection("");setSelectedTags([]);setFiles([]);setVideo("");setSpeciesId("");setPlantId("");setBusy(false);router.refresh();window.dispatchEvent(new Event("community-posted"));
    }catch(err){
  ...
  return <div className="panel overflow-hidden rounded-3xl">
    ...
    <div className="p-5">
      <label className="text-xs text-white/40">Post in a section
        <select value={section} onChange={event=>setSection(event.target.value)} className="mt-2 w-full rounded-xl border border-white/[.07] bg-[#08130e] px-3 py-2.5 text-sm text-white/65">
      ...
      <textarea value={text} onChange={event=>setText(event.target.value.slice(0,1200))} rows={4} placeholder={type==="QUESTION"?"Ask the keeper community something useful...":type==="BREEDING_UPDATE"?"Share a pairing, clutch, hatch or breeding observation...":"Share an update with the keeper community..."} className="mt-4 w-full resize-none bg-transparent text-sm leading-6 text-white outline-none placeholder:text-white/22"/>
      <div className="mt-3">
        <div className="mb-2 text-[10px] font-bold uppercase tracking-[.12em] text-white/24">Optional topic tags</div>
        <div className="flex flex-wrap gap-2">{LEGACY_COMMUNITY_TOPICS.map(tag=><button type="button" key={tag} onClick={()=>toggleTag(tag)} className={`rounded-full border px-3 py-1.5 text-[10px] ${selectedTags.includes(tag)?"border-emerald-300/20 bg-emerald-300/[.07] text-emerald-200":"border-white/[.07] text-white/30"}`}>{tag}</button>)}</div>
      </div>
