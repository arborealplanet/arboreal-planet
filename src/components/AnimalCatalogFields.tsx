...
export function AnimalCatalogFields({speciesId:initialSpeciesId="",localityId:initialLocalityId=""}:{speciesId?:string;localityId?:string}){
  ...
  const effectiveLocalityId=localityId&&available.some(x=>x.id===localityId)?localityId:"";
  ...
  return <>
    ...
    <label className="text-xs text-white/45">Locality<select name="locality_id" value={effectiveLocalityId} onChange={e=>setLocalityId(e.target.value)} disabled={!speciesId} className={field}><option value="">Unspecified</option>{available.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>
  </>;
}
