...
export function GtpBreederConfirmations() {
  ...
  const [breederUsername, setBreederUsername] = useState("");
  const [signedIn, setSignedIn] = useState<boolean | null>(null);
  const [status, setStatus] = useState("Checking breeder confirmations…");
  ...
  async function load() {
    try {
      ...
      if (animalsResponse.status === 401 || requestsResponse.status === 401) {
        setSignedIn(false);
        setStatus("Sign in to request or respond to breeder confirmations.");
        return;
      }
      ...
      setStatus(pending ? `${pending} pending breeder confirmation request${pending === 1 ? "" : "s"}.` : "No pending breeder confirmations.");
    } catch (error) {
      setSignedIn(null);
      setStatus(error instanceof Error ? error.message : "Could not load breeder confirmations.");
    }
  }
  ...
  const history = requests.filter((request) => request.confirmation_status !== "pending").slice(0, 10);

  async function createRequest(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!animalId || !breederUsername.trim()) return;
    setBusy(true);
    setStatus("Sending breeder confirmation request…");
    try {
      const response = await fetch("/api/genetics/pedigree/confirmations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ animalId, breederUsername: breederUsername.trim() }),
      });
  ...
  return <section className="panel rounded-[26px] p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="section-kicker">Breeder confirmation</div><h2 className="mt-2 text-xl font-semibold text-white/80">Ask the producer to confirm a pedigree record.</h2><p className="mt-2 max-w-3xl text-xs leading-5 text-white/40">Confirmation requires a second Arboreal Planet account to actively accept the request. It confirms that account&apos;s breeder relationship to the animal; it does not independently prove geographic locality.</p></div><span className="rounded-full border border-white/[.08] px-3 py-1.5 text-[10px] font-bold text-white/42">{signedIn ? "ACCOUNT LINKED" : signedIn === false ? "SIGN IN REQUIRED" : "CHECKING"}</span></div>
    ...
    {signedIn === false ? <Link href="/login?next=/genetics" className="primary-action mt-4 inline-block !min-h-0 !px-4 !py-2.5 !text-xs">Sign in</Link> : signedIn ? <>
      <form onSubmit={createRequest} className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
        ...
        <label className="text-[10px] font-bold uppercase tracking-[.1em] text-white/28">Breeder username<input value={breederUsername} onChange={(event) => setBreederUsername(event.target.value)} placeholder="@username" className="mt-2 w-full rounded-xl border border-white/[.08] bg-black/20 p-3 text-sm normal-case tracking-normal text-white/70" /></label>
        <button disabled={busy || !animals.length || !breederUsername.trim()} className="self-end rounded-xl bg-emerald-300 px-4 py-3 text-xs font-black text-[#06100c] disabled:opacity-35">Request confirmation</button>
      </form>

      {incoming.length ? <div className="mt-6"><div className="text-[10px] font-black uppercase tracking-[.13em] text-amber-200/60">Requests for you to confirm</div><div className="mt-3 grid gap-3 md:grid-cols-2">{incoming.map((request) => <div key={request.request_id} className="panel-soft rounded-2xl p-4"><div className="font-semibold text-white/68">{request.animal_name}</div><div className="mt-1 font-mono text-[10px] text-emerald-200/45">{request.registry_code}</div><div className="mt-3 text-xs text-white/38">Requested by {request.requester_display_name || request.requester_username || "keeper"}</div><div className="mt-4 flex gap-2"><button disabled={busy} onClick={() => void act(request.request_id, "confirm")} className="rounded-xl bg-emerald-300 px-3 py-2 text-[10px] font-black text-[#06100c] disabled:opacity-35">Confirm I produced this animal</button><button disabled={busy} onClick={() => void act(request.request_id, "decline")} className="rounded-xl border border-white/[.08] px-3 py-2 text-[10px] font-bold text-white/48 disabled:opacity-35">Decline</button></div></div>)}</div></div> : null}

      {outgoing.length ? <div className="mt-6"><div className="text-[10px] font-black uppercase tracking-[.13em] text-white/30">Waiting for breeder</div><div className="mt-3 space-y-2">{outgoing.map((request) => <div key={request.request_id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/[.055] p-3 text-xs"><div><span className="font-semibold text-white/58">{request.animal_name}</span><span className="ml-2 text-white/28">→ @{request.breeder_username || "breeder"}</span></div><button disabled={busy} onClick={() => void act(request.request_id, "cancel")} className="text-[10px] font-bold text-red-200/55 disabled:opacity-35">Cancel request</button></div>)}</div></div> : null}

      {history.length ? <details className="mt-5 rounded-2xl border border-white/[.055] p-4"><summary className="cursor-pointer text-xs font-semibold text-white/45">Confirmation history</summary><div className="mt-3 space-y-2">{history.map((request) => <div key={request.request_id} className="flex justify-between gap-3 text-xs text-white/35"><span>{request.animal_name}</span><span className="uppercase">{request.confirmation_status}</span></div>)}</div></details> : null}
    </> : null}
  </section>;
}
