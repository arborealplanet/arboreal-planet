...
export function AccountProfileEditor({email,initial}:{email:string;initial:Profile}){
 const router=useRouter(); const [profile,setProfile]=useState<Profile>(initial); const [saving,setSaving]=useState(false); const [uploading,setUploading]=useState<MediaField|null>(null); const [verifying,setVerifying]=useState(false); const [savingAccent,setSavingAccent]=useState(false); const [message,setMessage]=useState<string|null>(null); const accentRequest=useRef(0);
