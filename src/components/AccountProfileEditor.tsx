...
export function AccountProfileEditor({ email, initial }: { email: string; initial: Profile }) {
  ...
  async function upload(event:ChangeEvent<HTMLInputElement>,bucket:"avatars"|"profile-banners",field:"avatar_url"|"banner_url"){
    ...
    try{
      ...
      setMessage("Image uploaded. Save profile to keep the change.");
    }catch{
  ...
  async function save(){
    ...
    try{
      ...
      const data=await response.json().catch(()=>({}));
      if(!response.ok){setMessage(data.error??"Could not save profile.");return;}
      setMessage("Profile saved.");
      router.refresh();
    }catch{
      setMessage("Could not save profile. Check your connection and try again.");
    }finally{
      setSaving(false);
    }
  }
