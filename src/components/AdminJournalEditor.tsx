...
export function AdminJournalEditor(){
  const [articles,setArticles]=useState<Article[]>([]),[species,setSpecies]=useState<RefItem[]>([]),[plants,setPlants]=useState<RefItem[]>([]),[form,setForm]=useState<Form>(blank),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[uploadBusy,setUploadBusy]=useState(false),[message,setMessage]=useState(""),[preview,setPreview]=useState(false),[filter,setFilter]=useState("ALL");
  ...
  async function uploadCover(file:File){
    ...
    setUploadBusy(true);setMessage("Uploading cover image…");
    try{const payload=new FormData();payload.set("file",file);const response=await fetch("/api/admin/journal/media",{method:"POST",body:payload});const data=await response.json().catch(()=>null) as {url?:string;error?:string}|null;if(!response.ok||!data?.url)throw new Error(data?.error||"Could not upload cover image.");update("cover_image_url",data.url);setMessage("Cover image uploaded. Save the Journal piece to keep this change.")}catch(error){setMessage(error instanceof Error?error.message:"Could not upload cover image.")}finally{setUploadBusy(false)}
  }
  ...
  return <div className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)]">
    ...
    <section className="panel rounded-[26px] p-5 sm:p-6">
      ...
      {preview?<div className="mt-6 overflow-hidden rounded-[24px] border border-white/[.06] bg-black/10">{form.cover_image_url?<img src={form.cover_image_url} alt="" className="max-h-[360px] w-full border-b border-white/[.055] object-cover"/>:null}<div className="p-6 sm:p-8"><div className="text-[9px] font-black uppercase tracking-[.13em] text-emerald-200/55">{form.content_type} · {form.category}</div><h2 className="mt-3 text-3xl font-semibold tracking-[-.035em] text-white/82">{form.title||"Untitled Journal piece"}</h2>{form.excerpt?<p className="mt-4 text-sm leading-7 text-white/45">{form.excerpt}</p>:null}<div className="mt-6 whitespace-pre-wrap border-t border-white/[.055] pt-6 text-sm leading-8 text-white/58">{form.body||"No article body yet."}</div></div></div>:<div className="mt-6 grid gap-4">
        ...
        <div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-semibold text-white/45">Author display<input value={form.author_display} onChange={e=>update("author_display",e.target.value)} className="mt-2 w-full rounded-xl border border-white/[.08] bg-black/15 px-3 py-2.5 text-sm text-white/70 outline-none" /></label><div><div className="text-xs font-semibold text-white/45">Cover image</div><div className="mt-2 rounded-xl border border-white/[.08] bg-black/10 p-3"><input type="file" accept="image/jpeg,image/png,image/webp" disabled={uploadBusy} onChange={e=>{const file=e.target.files?.[0];if(file)void uploadCover(file);e.currentTarget.value=""}} className="block w-full text-xs text-white/42 file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-300/[.08] file:px-3 file:py-2 file:text-[10px] file:font-black file:text-emerald-100/65"/><input value={form.cover_image_url} onChange={e=>update("cover_image_url",e.target.value)} placeholder="Or paste an image URL" className="mt-3 w-full rounded-lg border border-white/[.06] bg-black/15 px-3 py-2 text-xs text-white/55 outline-none" />{form.cover_image_url?<img src={form.cover_image_url} alt="Cover preview" className="mt-3 h-28 w-full rounded-lg object-cover"/>:null}</div></div></div>
      ...
      <div className="mt-6 flex flex-wrap gap-2 border-t border-white/[.055] pt-5"><button type="button" disabled={busy||uploadBusy} onClick={()=>void save("save")} className="primary-action !min-h-0 !px-4 !py-2.5 !text-xs">Save draft</button><button type="button" disabled={busy||uploadBusy} onClick={()=>void save("publish")} className="rounded-xl border border-emerald-300/18 bg-emerald-300/[.04] px-4 py-2.5 text-xs font-black text-emerald-100/70 disabled:opacity-40">Publish</button>{form.id&&form.status!=="ARCHIVED"?<button type="button" disabled={busy||uploadBusy} onClick={()=>void save("archive")} className="rounded-xl border border-amber-300/15 px-4 py-2.5 text-xs font-black text-amber-100/55 disabled:opacity-40">Archive</button>:null}<span className="ml-auto self-center text-[10px] font-black uppercase tracking-[.12em] text-white/24">{uploadBusy?"UPLOADING":form.status}</span></div>
    </section>
  </div>;
}
