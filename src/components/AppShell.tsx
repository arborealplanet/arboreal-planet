...
export async function AppShell({ children }: { children: ReactNode }) {
    ...
    <header className="sticky top-0 z-50 border-b border-white/[.065] bg-[#06100c]/88 shadow-[0_10px_35px_rgba(0,0,0,.12)] backdrop-blur-xl">
      <div className="mx-auto flex min-h-[60px] max-w-[1440px] items-center gap-2 px-3 sm:min-h-[72px] sm:gap-5 sm:px-6">
        ...
        <nav aria-label="Primary" className="ml-auto hidden items-center gap-1 text-[13px] font-semibold text-white/60 xl:flex">
          {nav.map(([label, href]) => (
            <Link key={href} href={href} className="rounded-xl px-2.5 py-2.5 transition hover:bg-white/[.04] hover:text-white">{label}</Link>
          ))}
        </nav>
        ...
        <div className="ml-auto flex items-center gap-2 xl:ml-2">
          ...
          <PwaInstallButton />
          {signedIn&&<div className="xl:hidden"><NotificationBell /></div>}
        </div>
      ...
      <nav aria-label="Browse" className="hide-scrollbar mx-auto flex max-w-[1440px] gap-2 overflow-x-auto border-t border-white/[.04] px-3 py-2 text-center text-[10px] font-bold uppercase tracking-[.08em] text-white/50 xl:hidden sm:px-6">
  ...
  const footer = (
    <footer className="border-t border-white/[.06] bg-black/[.12] px-5 py-10 pb-24 xl:pb-10">
...
export function PageIntro({ eyebrow, title, description, aside }: { eyebrow: string; title: string; description: string; aside?: ReactNode }) {
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-5 pb-9 pt-10 sm:px-6 xl:grid-cols-[1fr_auto] xl:items-end xl:pt-14">
      <div>
