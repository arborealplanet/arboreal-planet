...
function projectedTraits(dam: Snake, sire: Snake) {
  return Object.fromEntries(traitKeys.map((key) => [key, Math.round((Number(dam[key] ?? 0) + Number(sire[key] ?? 0)) / 2)])) as Record<TraitKey, number>;
}
...
export function ChondroPairingPlanner() {
  ...
  const preview = useMemo(() => {
    ...
    const traits = projectedTraits(dam, sire);
    const kind = clutchPairingKind(dam, sire);
    const clutch = CLUTCH_SIZE_PROFILES[kind];
    const subspecies = dam.subspecies === sire.subspecies ? dam.subspecies : dam.subspecies;
    const candidate = { subspecies, classification, locality, generation, neonateColor: dam.neonateColor, ...traits };
    ...
    const focusMatch = traitFocusMatch(focus, traits);
    return { classification, locality, generation, traits, clutch, projects, focusMatch };
  }, [dam, sire]);
          ...
          {!preview ? <div className="mt-3 text-xs text-white/35">Choose an adult dam and sire to preview the pairing.</div> : (
            <div className="mt-3 space-y-3">
              ...
              <div className="rounded-xl border border-white/[.06] p-3">
                <div className="text-[9px] uppercase tracking-[.13em] text-white/25">Projected trait midpoint</div>
                <div className="mt-2 grid grid-cols-5 gap-1 text-center">
                  {traitKeys.map((key) => <div key={key}><div className="text-[8px] text-white/25">{traitLabel[key].replace("High ", "H ")}</div><div className="mt-1 text-xs font-bold text-white/65">{preview.traits[key]}%</div></div>)}
                </div>
                <div className="mt-2 text-[9px] leading-4 text-white/25">Midpoints are planning estimates, not guaranteed offspring percentages.</div>
              </div>
              {preview.focusMatch.matchedTargets.length ? <div className="text-[10px] text-amber-100/60">Trait Focus: {preview.focusMatch.matchedTargets.map((key) => TRAIT_FOCUS_LABELS[key]).join(", ")} · score {preview.focusMatch.score}%</div> : null}
              {preview.projects.length ? <div className="text-[10px] text-emerald-100/60">Projected project matches: {preview.projects.map((p) => p.name).join(" · ")}</div> : null}
