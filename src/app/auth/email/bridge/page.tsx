...
export default function EmailConfirmationBridgePage() {
  ...
  useEffect(() => {
    ...
    const invalidTarget = `/login?authError=invalid_confirmation&next=${encodeURIComponent(next)}`;
    ...
    if (errorDescription || !accessToken || !refreshToken) {
      window.location.replace(invalidTarget);
      return;
    }
  ...
  return (
    <main className="mx-auto max-w-xl px-5 py-20 sm:px-6">
      <div className="panel rounded-[30px] p-8 text-center">
        ...
        <p className="mt-3 text-sm text-white/40">Finishing email confirmation…</p>
      </div>
    </main>
  );
}
