...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-5 py-10 pb-20 sm:px-6 lg:py-14">
    ...
    {term && !items.length ? <div className="panel mt-5 rounded-[26px] py-16 text-center"><div className="font-semibold text-white/60">Nothing public matched that search.</div><p className="mt-2 text-xs text-white/32">Try a broader species, locality, topic, plant group, event, registry ID or keeper name.</p></div> : null}
