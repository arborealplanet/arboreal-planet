...
export default async function NewListingPage({searchParams}:{searchParams:Promise<{pedigree?:string|string[]}>}){
  ...
  const pedigreeParam=Array.isArray(params.pedigree)?params.pedigree[0]:params.pedigree;
  const initialPedigreeId=typeof pedigreeParam==="string"?pedigreeParam:"";
  return <main><PageIntro eyebrow="Marketplace · New listing" title="List it on Arboreal Planet." description="Create a real marketplace listing attached to your verified seller account. Animal origin uses the same Captive Bred / Import language used throughout Arboreal Planet."/><section className="mx-auto max-w-3xl px-5 pb-16 sm:px-6"><CreateListingForm initialPedigreeId={initialPedigreeId}/></section></main>;
}
