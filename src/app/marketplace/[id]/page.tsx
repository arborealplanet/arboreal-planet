...
type Pedigree={id:string;registry_code:string;name:string;locality_label:string|null}|null;
    ...
    <div className="mt-5 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
      <section className="panel overflow-hidden rounded-3xl">
        ...
        <div className="p-6 sm:p-8">
          ...
          {pedigree&&<div className="mt-6 rounded-2xl border border-emerald-300/12 bg-emerald-300/[.03] p-4"><div className="text-[9px] font-black uppercase tracking-[.14em] text-emerald-200/45">Arboreal Planet registered pedigree</div><div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><div className="font-semibold text-white/72">{pedigree.name}</div><div className="mt-1 font-mono text-[10px] font-bold tracking-[.08em] text-emerald-100/50">{pedigree.registry_code}{pedigree.locality_label?` · ${pedigree.locality_label}`:""}</div></div><Link href={`/genetics/database/${encodeURIComponent(pedigree.id)}`} className="rounded-xl bg-emerald-300 px-4 py-2.5 text-xs font-black text-[#06100c]">View Pedigree →</Link></div></div>}
