...
type Listing={id:string;owner_id:string;category:string;title:string;description:string|null;public_origin:string|null;price:number|null;currency:string;image_urls:string[];seller_location:string|null;morph:string|null;sex:string|null;age_or_year:string|null;lineage:string|null;status:string;created_at:string;species:{common_name:string;scientific_name:string}|null;localities:{name:string}|null;profiles:{username:string|null;display_name:string|null;avatar_url:string|null;seller_enabled:boolean}|null};

export default async function ListingPage({params}:{params:Promise<{id:string}>}){
  const {id}=await params;
  const identity=await getServerIdentity();
  const token=identity?.token??SUPABASE_AUTH_KEY;
  const r=await fetch(`${SUPABASE_AUTH_URL}/rest/v1/marketplace_listings?id=eq.${encodeURIComponent(id)}&select=id,owner_id,category,title,description,public_origin,price,currency,image_urls,seller_location,morph,sex,age_or_year,lineage,status,created_at,species!marketplace_listings_species_id_fkey(common_name,scientific_name),localities!marketplace_listings_locality_id_fkey(name),profiles!marketplace_listings_owner_id_fkey(username,display_name,avatar_url,seller_enabled)`,{headers:{apikey:SUPABASE_AUTH_KEY,Authorization:`Bearer ${token}`},cache:"no-store"});
  if(!r.ok)notFound();
