...
export default async function SavedPage() {
  ...
  const total = animals.length + plants.length + listings.length + journal.length + events.length + posts.length + keepers.length;

  return <main>
    <PageIntro eyebrow="Your Arboreal Planet" title="Saved & Following" description="Your personal corner of Arboreal Planet: watched reference records, Journal pieces, events, listings, saved Community posts and keepers you follow." aside={<div className="rounded-full border border-emerald-300/15 bg-emerald-300/[.04] px-4 py-2 text-[10px] font-black uppercase tracking-[.14em] text-emerald-100/65">{total} saved connection{total===1?"":"s"}</div>} />
    <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-6">
      ...
      {events.length?<section className="mb-10"><div className="mb-4 flex items-end justify-between"><div><div className="section-kicker">Shows & Events</div><h2 className="mt-2 text-2xl font-semibold">Saved events</h2></div><Link href="/events" className="text-xs font-bold text-emerald-200/60">Events →</Link></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{events.map((item)=><Link key={item.id} href={`/events/${item.slug}`} className="panel interactive-card rounded-[22px] p-5"><div className="text-[9px] font-black uppercase tracking-[.13em] text-emerald-200/50">{item.event_type.replaceAll("_"," ")}</div><div className="mt-2 text-lg font-semibold text-white/72">{item.title}</div><div className="mt-3 text-xs leading-5 text-white/34">{new Date(item.starts_at).toLocaleDateString()} · {[item.venue_name,item.city,item.state_region,item.country].filter(Boolean).join(" · ")}</div><div className="mt-4 text-xs font-bold text-emerald-200/50">Open event →</div></Link>)}</div></section>:null}
