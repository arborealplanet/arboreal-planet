...
type WatchRow = { item_type: "ANIMAL" | "PLANT" | "MARKET_LISTING"; item_id: string; created_at: string };
...
export default async function SavedPage() {
  ...
  const aIds = ids(animalIds, "item_id"), pIds = ids(plantIds, "item_id"), lIds = ids(listingIds, "item_id"), postIds = ids(savedPosts, "post_id"), keeperIds = ids(follows, "following_id");

  const [animals, plants, listings, posts, keepers] = await Promise.all([
  ...
  const total = animals.length + plants.length + listings.length + posts.length + keepers.length;

  return <main>
    <PageIntro eyebrow="Your Arboreal Planet" title="Saved & Following" description="Your personal corner of Arboreal Planet: watched reference records, listings, saved Community posts and keepers you follow." aside={<div className="rounded-full border border-emerald-300/15 bg-emerald-300/[.04] px-4 py-2 text-[10px] font-black uppercase tracking-[.14em] text-emerald-100/65">{total} saved connection{total===1?"":"s"}</div>} />
    <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-6">
      {total===0?<div className="panel rounded-[28px] py-16 text-center"><div className="text-lg font-semibold text-white/65">Nothing saved yet.</div><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-white/34">Follow keepers, save Community posts, or add animal and plant references to your watchlist as you explore the site.</p><div className="mt-5 flex flex-wrap justify-center gap-3"><Link href="/search" className="primary-action">Search the hub</Link><Link href="/community" className="secondary-action">Explore Community</Link></div></div>:null}
