...
export async function POST(request:NextRequest){
  ...
  for(const file of files){
    if(!allowed.has(file.type)||file.size>10*1024*1024)return NextResponse.json({error:"Images must be JPG, PNG, or WebP and no larger than 10 MB each"},{status:400});
    const ext=file.type==="image/png"?"png":file.type==="image/webp"?"webp":"jpg";
    const path=`${identity.user.id}/${crypto.randomUUID()}.${ext}`;
    const upload=await fetch(`${SUPABASE_AUTH_URL}/storage/v1/object/marketplace/${path}`,{method:"POST",headers:{apikey:SUPABASE_AUTH_KEY,Authorization:`Bearer ${identity.token}`,"Content-Type":file.type,"x-upsert":"false"},body:await file.arrayBuffer()});
    if(!upload.ok)return NextResponse.json({error:"Photo upload failed"},{status:400});
