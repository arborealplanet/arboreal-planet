...
export async function GET() {
  ...
  const profileIds = [...new Set(animals.flatMap((row) => [String(row.owner_id ?? ""), String(row.breeder_profile_id ?? "")]).filter(Boolean))];
  const profiles = new Map<string, PublicProfile>();

  if (profileIds.length) {
    const profileResponse = await fetch(`${SUPABASE_AUTH_URL}/rest/v1/profiles?id=in.(${profileIds.join(",")})&select=id,username,display_name,avatar_url`, {
      headers: {
        apikey: SUPABASE_AUTH_KEY,
        Accept: "application/json",
      },
  ...
  return NextResponse.json({
    animals: animals.map((row) => {
      ...
      const breeder = profiles.get(String(row.breeder_profile_id ?? ""));
      return {
        id: row.id,
        ...
        confirmedBreeder: breeder ? {
          username: breeder.username ?? null,
          displayName: breeder.display_name ?? null,
          avatarUrl: breeder.avatar_url ?? null,
        } : null,
