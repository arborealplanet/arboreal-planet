...
export async function GET(request: NextRequest) {
  ...
  const [animalsResponse, mediaResponse] = await Promise.all([
    ...
    fetch(`${SUPABASE_AUTH_URL}/rest/v1/snake_sorter_reference_media?select=id,animal_id,storage_path,original_name,mime_type,created_at&order=created_at.asc`, { headers: headers(identity.token), cache: "no-store" }),
  ]);
  ...
  const columns = ["animal_id","animal_code","media_id","storage_path","original_name","mime_type","taxon","locality","life_stage","neonate_color","label_confidence","purity_status","dataset_split","source_type","source_name","review_notes"];
